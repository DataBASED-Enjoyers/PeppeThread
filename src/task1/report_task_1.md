...
Ниже представлены результаты в табличном виде. Для каждой фиксированной размерности задачи ($N$) варьировалось число потоков (`threads`), и замерялось среднее общее время выполнения (`avg_time`) и CUDA (`avg_time_CUDA`), а также рассчитывалось ускорение ($S$).
