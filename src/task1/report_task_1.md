...
$`
S = \frac{T_{\text{serial}}}{T_{\text{multithreads}}}
`$
...
Ниже представлены результаты в табличном виде. Для каждой фиксированной размерности задачи ($N$) варьировалось число потоков (`threads`), и замерялось среднее время выполнения на CPU (`avg_time_CPU`) и GPU (`avg_time_CUDA`), а также рассчитывалось ускорение ($S$).
